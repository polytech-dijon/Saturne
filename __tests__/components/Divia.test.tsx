...
describe('Divia Component', () => {
  ...
  test('displays error message when data fetch fails', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Erreur lors du chargement des données de transport')).toBeInTheDocument();
      expect(screen.getByText('Impossible de charger les données de transport')).toBeInTheDocument();
    });
  });
  ...
  test('displays error when incomplete data is received', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Erreur lors du chargement des données de transport')).toBeInTheDocument();
      expect(screen.getByText('Données de station incomplètes reçues.')).toBeInTheDocument();
    });
  });
